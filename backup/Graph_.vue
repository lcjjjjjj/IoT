...
export default {
  ...
  mounted() {
    request({
      ...
      const filteredData = this.filterGraphData(res.data, 30);
  ...
  methods: {
    filterGraphData(data, maxRelations) {
      if (!data || !data.edges || data.edges.length <= maxRelations) {
        return data;
      }
      const filteredData = {
        vertexLists: [],
        edges: data.edges.slice(0, maxRelations),
      };
      ...
      filteredData.vertexLists = data.vertexLists.filter(vertex => nodeIds.has(vertex.VID));
      return filteredData;
    },
    createNodes(vertexs) {
      var nodes = []
